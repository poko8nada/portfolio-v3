...
- Every TODO must include:
  - What to implement (one-line summary)
  - Hints (relevant variables, functions, or concepts to use)
  - File path and line number of related existing code, if applicable

```typescript
// TODO(human): Implement the filtering logic
// - Return all tasks if filterText is empty
// - Return only tasks whose title contains filterText (case-insensitive)
// Hint: use Array.filter() and String.toLowerCase().includes()
```
