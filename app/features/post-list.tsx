...
import { parseMetadata, type PostMetadata } from '../lib/markdown';
...
export const PostList = async ({
  ...
  const metadataList: Array<{ slug: string } & PostMetadata> = [];
  ...
  return (
    <ul class='list-none px-0'>
      {sortedMetadataList
        ...
        .map((meta) => (
          <ListContent key={meta.slug} href={`/posts/${meta.slug}`} title={meta.title}>
            <div class='mt-2 text-sm text-text-secondary'>
              {new Date(meta.createdAt).toLocaleDateString('ja-JP', {})}
            </div>
            <div class='mt-1'>
              {meta.tags?.map((postTag) => (
                <Button key={postTag} size='sm' href={`/posts?tag=${postTag}`}>
                  {postTag}
                </Button>
              ))}
            </div>
