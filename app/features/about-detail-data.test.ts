...
describe('about detail data', () => {
  ...
  it('groups stack items by genre in source order', () => {
    ...
    expect(groups[0]?.label).toBe('Markup');
    expect(groups[1]?.label).toBe('Frontend');
  ...
  it('groups stack items by frequency in configured order', () => {
    ...
    expect(groups.map((group) => group.label)).toEqual(['Daily', 'Often', 'Sometimes']);
    expect(groups[1]?.items.some((item) => item.label === 'React')).toBe(true);
    expect(groups[0]?.items.some((item) => item.label === 'HeroUI')).toBe(true);
  });
});
