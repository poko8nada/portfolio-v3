...
export function Resume(props: { data: ResumeData }) {
  // TODO(human): Build the Japanese 履歴書 layout: two A4-portrait sections with screen and `@media print` styles so each section prints as one page and two pages side-by-side read like an A3 spread (`docs/adr/0008-resume-page-from-r2-json-with-cloudflare-access.md`). Force white background and black text; do not rely on the site header/footer (see `app/routes/_renderer.tsx` for `/resume`).
  // Hint: Map `props.data` fields to sections (氏名・住所・学歴・職歴・免許・志望動機等). Compose `ResumePhoto` with `props.data.profile.photo`.
  ...
  return (
    <div data-resume-page-root class='bg-white text-black serif-only w-3xl mx-auto mt-6'>
      <div class={'m-2'}>
