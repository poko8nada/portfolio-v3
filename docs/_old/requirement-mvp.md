...
### 9.1. 運用要件（GA4/GTM とカスタムドメイン移行）

- 既存ポートフォリオ v2 から、GA4（GTM経由）とカスタムドメイン設定を移行する。
- 作業は「ダッシュボード作業」と「コード作業」を分離して管理する。

**ダッシュボード作業（GTM/GA4）**

- 既存GTMコンテナのタグ/トリガー/変数を棚卸しし、移設対象を確定する。
- 本番用GTMコンテナへGA4関連タグを移設し、Previewで発火確認する。
- GA4 Realtimeでイベント受信を確認後、GTMをPublishする。

**コード作業（アプリ側）**

- GTMスニペットの埋め込みをアプリの共通レイアウトで管理する。
- GTM/GA4用の識別子は環境変数で管理し、ハードコードしない。
- 環境差分（local/preview/prod）で設定を切り替えられる構成にする。

**カスタムドメイン作業（Cloudflare管理）**

- Cloudflare DNS/Workers Routesに既存ドメイン設定を引き継ぐ。
- SSL/TLS証明書状態とリダイレクト方針（www有無）を確認する。
- 公開URLでルーティング・計測・アセット配信を確認する。
