**This is a template. Fill in the [] or ``, duplicate or delete the template parts as needed.**

# MVP実装タスク (tasks-mvp.md)

## Phase 1: [フェーズ名]

### Task 0: 依存関係のセットアップ

- [ ] 必要なパッケージのインストール
  - `pnpm add [パッケージ名1] [パッケージ名2] [...]`
  - `pnpm add -D [開発用パッケージ名1] [開発用パッケージ名2] [...]`
- [ ] `package.json` へのスクリプト追加
  - `"test": "[テスト実行コマンド]"`
  - `"lint": "[Lint/フォーマット実行コマンド]"`
  - `"format": "[コードフォーマット実行コマンド]"`
  - `"typecheck": "[型チェックコマンド]"`
    など
- [ ] 開発環境の動作確認
  - `pnpm run dev` でエラーが出ないことを確認
- [ ] 不要なファイル・コードの削除
  - 初期テンプレートのサンプルコードなど

**依存関係**: なし
**成果物**: 開発に必要なライブラリとスクリプトが利用可能な状態
**完了条件**: [タスク完了を判断する基準]
**テスト**: 不要

### Task 1

- [ ] [機能単位の作業項目名]
- [ ] `ファイルパス/ファイル名.ts` - [変更内容] **(FR-XX | fn: functionName | type: TypeName)**
- [ ] `ファイルパス/ファイル名.test.ts` - [テスト内容] **(FR-XX | fn: functionName)**
- [ ] `ファイルパス/ファイル名.tsx` - [変更内容] **(FR-XXa | cmp: ComponentName | type: ComponentProps)**

**依存関係**: [なし / Task X, Task Y]
**成果物**: [このタスクで達成される具体的な成果物]
**完了条件**: [タスク完了を判断する基準]
**テスト**: [テスト方針 - 単体テスト必須/不要/手動確認]

### FRトレーサビリティ表（任意だが推奨）

| FR ID  | 機能名       | 対象ファイル                    | 対応Task       |
| ------ | ------------ | ------------------------------- | -------------- |
| FR-01  | [機能名]     | `app/.../a.ts`, `app/.../b.tsx` | Task 1, Task 2 |
| FR-01a | [サブ機能名] | `app/.../a.ts`                  | Task 1         |

> 可能なら各Taskのファイル行に `fn: / cmp: / type:` を併記し、FR→実装I/Fの追跡を容易にする。

**Note:** This is a task template. Duplicate or delete as needed.

### Task [No]

### Phase [No]: [フェーズ名]

### Task [No]

**Note:** This is a task template. Duplicate or delete as needed. Loop as needed for additional phases and tasks.

---

## Phase [No]: 検証・最適化・デプロイ (Validation & Deployment)

### Task [No]: ブラウザ互換性・パフォーマンス確認

- [ ] ブラウザ互換性テスト（手動確認）
  - Chrome latest: 全機能動作確認
  - Firefox latest: 全機能動作確認
  - Safari latest: 全機能動作確認
- [ ] パフォーマンス測定
  - [パフォーマンス要件1]
  - [パフォーマンス要件2]
- [ ] アクセシビリティ監査（axe DevTools）
  - WCAG AA 基準クリア
- [ ] 最小限の単体テスト実行

**依存関係**: 機能実装完了
**完了条件**: 全ブラウザで動作確認完了、最小限のテストがパス
**注**: E2E テストは実施しない

### Task [No]: デプロイ準備・本番環境確認

- [ ] デプロイ設定
  - 環境変数設定（必要に応じて）
  - ビルド設定確認
- [ ] OGP 画像作成・配置
  - `og.png` (1200x630px)
  - メタデータ設定
- [ ] Favicon 準備・追加
  - `favicon.ico`
  - `icon.png` (該当する場合)
- [ ] 本番環境での動作確認
  - 全機能の動作確認
  - レスポンシブ表示確認
- [ ] 次段階への移行準備ドキュメント作成
  - [移行計画ドキュメント]
  - [設計書]

**依存関係**: ブラウザ互換性・パフォーマンス確認完了
**成果物**: デプロイ済み MVP、運用ドキュメント
**完了条件**: 本番環境で全機能が動作すること

---

## チェックリスト（デプロイ前最終確認）

### テスト

- [ ] 最小限の単体テストがパス（`pnpm run test`）
- [ ] 全ブラウザで手動動作確認完了（Chrome, Firefox, Safari）

### リソース・メタデータ

- [ ] OGP画像・Favicon設置完了
- [ ] [ライセンスクレジット表示確認（該当する場合）]
- [ ] メタデータ設定完了（title, description, OGP）

### 機能確認

- [ ] [機能A] 動作確認
- [ ] [機能B] 動作確認
- [ ] [機能C] 動作確認
- [ ] エラーハンドリング動作確認

### デプロイ

- [ ] 本番環境で全機能動作確認
- [ ] 環境変数設定確認（該当する場合）
- [ ] ビルド成功確認

### ドキュメント

- [ ] README.md 更新完了
- [ ] ライセンス情報記載完了
- [ ] デプロイ手順ドキュメント作成完了
